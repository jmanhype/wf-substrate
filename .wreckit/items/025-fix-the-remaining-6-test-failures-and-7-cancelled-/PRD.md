...
#### US-001: Execute test suite and catalog all failures and cancellations
**Priority**: 1 (Critical)

As a developer, I need to execute the full test suite and catalog all failures and cancellations so that I can understand exactly what needs to be fixed.
...
#### US-002: Fix verify_scope_isolation validation logic
**Priority**: 1 (Critical)

As a developer, I need to fix the `wf_cancel:verify_scope_isolation/2` validation logic so that it correctly validates that tokens within a cancelled scope are cancelled, while allowing active tokens in other scopes.
...
#### US-003: Fix timing-dependent performance test thresholds
**Priority**: 2 (High)

As a developer, I need to fix the timing-dependent performance test by increasing thresholds to account for CI environment variability so that tests pass consistently.
...
#### US-004: Add setup/teardown to modules lacking gen_server initialization
**Priority**: 2 (High)

As a developer, I need to add setup/cleanup functions and master test wrappers to test modules that depend on gen_servers so that tests execute with proper initialization.
...
**Notes**: Add setup to modules incrementally (one or two at a time) and verify after each addition. Test modules that DON'T use gen_servers (pure function tests) don't need setup. Use grep to identify which modules actually call wf_state:* or other gen_server functions.

---

#### US-005: Fix compilation errors causing cancelled tests
**Priority**: 3 (Medium)

As a developer, I need to fix compilation errors that are preventing tests from running so that all cancelled tests can execute and pass.
...
**Notes**: Common issues: missing -include_lib(\"eunit/include/eunit.hrl\"), missing wf_state.hrl, wf_exec.hrl, wf_cancel.hrl includes. Use grep to check what other working test files include. If a test is cancelled due to unimplemented functionality, note it but don't disable the test.

---

#### US-006: Final verification and full test suite validation
**Priority**: 1 (Critical)

As a developer, I need to verify that all fixes are working correctly and the full test suite passes with 0 failures and 0 cancelled tests so that the codebase is in a stable state.
...
**Notes**: This is the final validation phase. Only mark complete when ALL acceptance criteria are met. If any failures remain, investigate and fix before completing. The goal is 100% test pass rate with no cancelled tests.

---

## Implementation Order

1. **US-001** (Discovery) - Execute first, no dependencies
2. **US-002** (Logic Fix) - Can run in parallel with US-003, US-004
3. **US-003** (Timing Fix) - Can run in parallel with US-002, US-004
4. **US-004** (Setup Addition) - Can run in parallel with US-002, US-003
5. **US-005** (Compilation Fixes) - Depends on US-001 findings
6. **US-006** (Final Validation) - Must be last, depends on all previous

## Success Metrics

- **Before**: 6 failures, 7 cancelled tests
- **After**: 0 failures, 0 cancelled tests
- **Test Execution Time**: Should not increase significantly (setup overhead minimal)
- **CI/CD Pass Rate**: 100% consistent passes (no intermittent failures)
- **Code Coverage**: Should maintain or improve (no tests disabled)
